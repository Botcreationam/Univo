...
export default function ContactSection() {
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    if (!validate()) return
    setLoading(true)
    // Simulate submission (replace with real API endpoint when backend is ready)
    ...
    <section id="contact" className="section-py bg-navy-50 dark:bg-navy-900/30">
      <div className="max-w-6xl mx-auto px-5 sm:px-6 lg:px-8" ref={ref}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            ...
            {[
              ...
              <div key={label} className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-navy-900 dark:text-white font-medium text-sm mt-0.5">{value}</div>
                </div>
              </div>
            ))}
          </motion.div>
          ...
          >
            {submitted ? (
              <div className="h-full flex flex-col items-center justify-center text-center py-16 px-8 rounded-2xl bg-white dark:bg-white/5 border border-navy-100 dark:border-white/10">
                ...
                <p className="text-navy-500 dark:text-navy-400 text-sm">We will be in touch within 24 hours. Welcome to the UNIVO community.</p>
                <button
                  onClick={() => { setSubmitted(false); setForm({ name: '', email: '', interest: 'Student', message: '' }) }}
              ...
              <form onSubmit={handleSubmit} className="space-y-4 p-6 rounded-2xl bg-white dark:bg-white/5 border border-navy-100 dark:border-white/10" noValidate>
